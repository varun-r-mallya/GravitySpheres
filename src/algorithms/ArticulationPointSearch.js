...
export default class ArticulationPointSearch {
  ...
  start(startNode) {
    ...
    return this.search(startNode).then(() => {
      ...
      if (unvisitedPoints.length === 0 && articulationPoints.length === 0) {
        ...
        if (unvisitedPoints.length > 0) {
          this.log(
            ` | Unvisited Points: ${unvisitedPoints
              .map((n) => n.id)
              .join(", ")}`
          );
        }
  ...
  search(node, parent) {
    ...
    return this.updateComponent(node, parent).then(() => {
          ...
          if (!child.visited) {
            ...
            return this.search(child, node).then(() => {
              ...
              if (node.visitedFrom === null && node.childVisitCount > 1) {
                this.log(
                  " | Node is root and has more than one child - it's an articulation point!"
                );
                node.articulationPoint = true;
              } else if (
                node.visitedFrom !== null &&
                child.lowLink >= node.visitIndex
              ) {
                this.log(` | Node ${node.id} - parent ${node.visitedFrom}`);
                this.log(
                  " | Node is not root but one of it's children has a back link to an ancestor - it's an articulation point!"
                );
                node.articulationPoint = true;
              }
            });
