...
export default class GraphContainer extends Component {
  ...
  render() {
    return <div className="graph-container" id={`graph${this.props.id}`}></div>;
  }
  ...
  getDimensions() {
    let svgElem = d3.select(".graph-container").node();
    return {
  ...
  draw() {
    ...
    d3.selectAll(`.graph-container#graph${self.props.id} svg`).remove();

    let svg = d3
      .select(`.graph-container#graph${self.props.id}`)
      .append("svg")
  ...
  drawComponent(component) {
    let svg = d3.select(`.graph-container#graph${this.props.id} svg`);
  ...
  drawNode(node) {
    ...
    let svg = d3.select(`.graph-container#graph${self.props.id} svg`);
    ...
    let drag = d3
      ...
      .on("start", function (d) {
        ...
        d3.select(`.graph-container#graph${self.props.id} svg`)
          .append("line")
      ...
      .on("drag", function (d) {
        let draggedLine = d3.select(
          `.graph-container#graph${self.props.id} svg .Edge.dragging`
        );
